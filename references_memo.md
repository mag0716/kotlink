...
* `actor`
  * コルーチンに閉じ込め、カプセル化された状態、および他のコルーチンと通知するためのチャネルの組み合わせ
  * 単純なものは関数として実装できるが、複雑なものはクラスが適している
  * 最初は sealed classes を使ってメッセージのクラスを定義すること
  * どのコンテキストで実行されるかは問題ではない
  * 別のコンテキストに切り替える必要がないため、ロック機構よりもより効果的
  * 二重の `produce` コルーチンビルダー
